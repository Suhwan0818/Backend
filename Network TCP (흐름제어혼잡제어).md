...
- 해결 방법  
    - stop and wait : 매번 전송한 패킷에 대한 확인 응답을 받아야지만 그 다음 패킷을 전송하는 방법  
![stop and wait](/img/stop%20and%20wait.png)  

    - Sliding Window (Go back and ARQ)  
        - 수신측에서 설정한 윈도우 크기만큼 송신측에서 확인 응답없이 세그멘트를 전송할 수 있게 하여 데이터 흐름을 동적으로 조절하는 제어기법  
        - 목적 : 전송은 되었지만, acked를 받지 못한 byte의 숫자를 파악하기 위해 사용하는 prortocol  
